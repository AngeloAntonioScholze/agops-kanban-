...
import 'package:boardview_example/board_list_object.dart';
import 'package:flutter/material.dart';

class BoardViewExample extends StatefulWidget {
  const BoardViewExample({super.key});

  @override
  BoardViewExampleState createState() => BoardViewExampleState();
}
      ...
      BoardCallbacks(
        ...
        onScrollStateChanged: (isScrolling) {
          setState(() {
            ...
            _lastAction =
                isScrolling ? 'Started scrolling' : 'Stopped scrolling';
          });
        },
        onAnimationStateChanged: (isAnimating) {
          setState(() {
            ...
            _lastAction =
                isAnimating ? 'Animation started' : 'Animation completed';
          });
        },
        onDragStart: (listIndex, itemIndex) {
          setState(() {
            _isDragging = true;
            _lastAction =
                'Started dragging item $itemIndex from list $listIndex';
          });
        },
        onDragEnd: (fromListIndex, fromItemIndex, toListIndex, toItemIndex) {
          setState(() {
            _isDragging = false;
            _lastAction =
                'Moved item from list $fromListIndex to list $toListIndex';
          });
        },
        onDragCancel: (listIndex, itemIndex) {
          setState(() {
            _isDragging = false;
            _lastAction =
                'Cancelled dragging item $itemIndex from list $listIndex';
          });
        },
        onItemReorder: (listIndex, fromIndex, toIndex) {
          setState(() {
            _lastAction =
                'Reordered item in list $listIndex from $fromIndex to $toIndex';
          });
        },
